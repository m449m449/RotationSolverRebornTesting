...
public partial class RotationConfigWindow : Window
{
	...
	private void DrawDutyCustomRotationOverride(float comboSize, ICustomRotation[] rotations)
	{
		...
		{
			DrawPreDutyTerritorySelector(comboSize, territoryType);
			territoryType = GetPreDutyConfigTerritory(job);
			if (territoryType == 0)
			{
				return;
			}
		...
		var territoryName = GetTerritoryDisplayName(territoryType);
		RotationUpdater.TryGetDutyCustomRotationChoice(territoryType, job, CombatType.PvE, out var dutyChoice);

		var previewName = GetRotationChoiceDisplayName(rotations, dutyChoice, $"{_curRotationAttribute.Name} (global)");

		ImGui.Separator();
		ImGui.TextWrapped(DataCenter.IsInDuty
			? $"Duty-specific rotation for {territoryName}"
			: $"Preconfigure duty-specific rotation for {territoryName}");
		ImGui.SetNextItemWidth(Math.Max(comboSize, 220 * Scale));
		...
		ImguiTooltips.HoveredTooltip(DataCenter.IsInDuty
			...
			: "Choose a duty and preconfigure a different rotation before entering it.");
	}
	...
	private void DrawDutyCustomRotationFieldTest(float comboSize, ICustomRotation[] rotations)
	{
		...
		RotationUpdater.TryGetDutyCustomRotationChoice(selectedTerritory, job, CombatType.PvE, out var dutyChoice);

		var previewName = "Disabled";
		if (hasSelectedTerritory)
		{
			previewName = GetTerritoryDisplayName(selectedTerritory);
			previewName += $" - {GetRotationChoiceDisplayName(rotations, dutyChoice, _curRotationAttribute.Name)}";
		}

		ImGui.Separator();
		ImGui.TextWrapped("Field test using a configured duty-specific rotation");
		ImGui.SetNextItemWidth(Math.Max(comboSize, 280 * Scale));
	...
	private static string GetRotationChoiceDisplayName(ICustomRotation[] rotations, string? choice, string fallback)
	{
		if (!string.IsNullOrWhiteSpace(choice))
		{
			foreach (var possibleRotation in rotations)
			{
				if (possibleRotation.GetType().FullName == choice)
				{
					return possibleRotation.GetAttributes()?.Name ?? possibleRotation.Name;
				}
			}
		}
	...
	private void DrawImportedTimelineSection(float comboSize)
	{
		if (DataCenter.IsPvP)
		{
			return;
		...
		ImGui.Separator();
		ImGui.TextWrapped("Imported duty timeline profiles");

		ImGui.SetNextItemWidth(Math.Max(comboSize, 340 * Scale));
		_ = ImGui.InputTextWithHint("##ImportedTimelinePath", "Path to extracted timeline profile JSON", ref _importedTimelineImportPath, 512);

		ImGui.SameLine();
		if (ImGui.Button("Import##ImportedTimeline"))
		{
			try
			{
				var profile = ImportedTimelineManager.ImportProfile(_importedTimelineImportPath);
				_selectedImportedTimelineProfileId = profile.ProfileId;
				Svc.Toasts.ShowNormal($"Imported timeline profile: {profile.ProfileName}");
			}
			catch (Exception ex)
			{
				PluginLog.Warning($"Failed to import timeline profile: {ex.Message}");
				Svc.Toasts.ShowNormal("Failed to import timeline profile. See plugin log for details.");
			}
		}
		ImguiTooltips.HoveredTooltip("Import a timeline profile from a local JSON file path.");

		ImGui.SameLine();
		if (ImGui.Button("Import Clipboard##ImportedTimeline"))
		{
			try
			{
				var clipboardText = ImGui.GetClipboardText();
				var profile = ImportedTimelineManager.ImportProfileFromText(clipboardText ?? string.Empty);
				_selectedImportedTimelineProfileId = profile.ProfileId;
				Svc.Toasts.ShowNormal($"Imported timeline profile from clipboard: {profile.ProfileName}");
			}
			catch (Exception ex)
			{
				PluginLog.Warning($"Failed to import timeline profile from clipboard: {ex.Message}");
				Svc.Toasts.ShowNormal("Failed to import timeline profile from clipboard. See plugin log for details.");
			}
		}
		ImguiTooltips.HoveredTooltip("Import a timeline profile by pasting the JSON body into your clipboard.");

		ImGui.SameLine();
		if (ImGui.Button("Refresh##ImportedTimeline"))
		{
			ImportedTimelineManager.LoadProfiles();
		}

		ImGui.SameLine();
		if (ImGui.Button("Open Folder##ImportedTimeline"))
		{
			_ = Process.Start("explorer.exe", $"\"{ImportedTimelineManager.ProfilesDirectory}\"");
		}
		ImguiTooltips.HoveredTooltip("Import a JSON profile generated by RotationSolver.TimelineExtractor, import directly from clipboard JSON, reload copied files, or open the storage folder.");

		var profiles = ImportedTimelineManager.Profiles;
		SyncSelectedImportedTimelineProfile(profiles);

		if (profiles.Length == 0)
		{
			ImGui.TextWrapped("No imported timeline profiles are loaded.");
			return;
		}

		ImGui.TextWrapped($"Loaded profiles: {profiles.Length}");
		ImGui.TextWrapped("Import only adds a timeline profile to the library. Assign a duty below before it can appear in field test.");
		...
		if (territoryType != 0)
		{
			ImportedTimelineManager.TryGetDutyTimelineProfileChoice(territoryType, job, CombatType.PvE, out var profileId);
			var territoryName = GetTerritoryDisplayName(territoryType);
			...
			ImguiTooltips.HoveredTooltip(DataCenter.IsInDuty
				...
				: "Choose a duty and assign one imported timeline profile before entering it.");
		}
		else
		{
			ImGui.TextWrapped("Select a duty above to assign an imported timeline profile.");
		}

		DrawImportedTimelineLibrary(comboSize, profiles);

		if (ImportedTimelineRuntime.TryGetActiveProfile(out var activeProfile) && activeProfile != null)
		{
			ImGui.TextWrapped($"Active imported timeline: {activeProfile.ProfileName}");
		}
	}
	...
	private void DrawImportedTimelineLibrary(float comboSize, ImportedTimelineProfile[] profiles)
	{
		...
		var selectedProfile = GetSelectedImportedTimelineProfile(profiles);
		if (selectedProfile != null)
		{
			DrawImportedTimelineLibraryEntry(selectedProfile);
		}
	...
	{
		ImGui.Spacing();
		ImGui.TextWrapped($"Selected profile: {GetImportedTimelineProfileLabel(profile)}");
		var assignments = ImportedTimelineManager.GetAssignments(profile.ProfileId);
		...
		else
		{
			...
			ImportedTimelineManager.TimelineProfileAssignment? assignmentToRemove = null;
			foreach (var assignment in assignments)
			{
				...
				ImGui.SameLine();
				if (ImGui.SmallButton($"Unassign##ImportedTimelineAssignment:{profile.ProfileId}:{assignment.TerritoryType}:{(int)assignment.Job}:{(int)assignment.CombatType}"))
				{
					assignmentToRemove = assignment;
				}
			}

			if (assignmentToRemove.HasValue)
			{
				var assignment = assignmentToRemove.Value;
				ImportedTimelineManager.SetDutyTimelineProfileChoice(assignment.TerritoryType, assignment.Job, assignment.CombatType, null);
				if (assignment.CombatType == CombatType.PvE
					&& Service.Config.DutyTimelineProfileTestTerritory == assignment.TerritoryType)
				{
					Service.Config.DutyTimelineProfileTestTerritory = 0;
				}
		...
		if (ImGui.Button($"Delete selected profile##ImportedTimelineDelete:{profile.ProfileId}"))
		{
			if (ImportedTimelineManager.DeleteProfile(profile.ProfileId))
			{
				if (string.Equals(_selectedImportedTimelineProfileId, profile.ProfileId, StringComparison.OrdinalIgnoreCase))
				{
					_selectedImportedTimelineProfileId = string.Empty;
				}
	...
	private static void SyncSelectedImportedTimelineProfile(ImportedTimelineProfile[] profiles)
	{
		if (profiles.Length == 0)
		{
			_selectedImportedTimelineProfileId = string.Empty;
		...
		if (!string.IsNullOrWhiteSpace(_selectedImportedTimelineProfileId)
			&& Array.Exists(profiles, profile => string.Equals(profile.ProfileId, _selectedImportedTimelineProfileId, StringComparison.OrdinalIgnoreCase)))
		{
			return;
		}

		_selectedImportedTimelineProfileId = profiles[0].ProfileId;
	}

	private static ImportedTimelineProfile? GetSelectedImportedTimelineProfile(ImportedTimelineProfile[] profiles)
	{
		if (profiles.Length == 0)
		{
			return null;
		...
		foreach (var profile in profiles)
		{
			if (string.Equals(profile.ProfileId, _selectedImportedTimelineProfileId, StringComparison.OrdinalIgnoreCase))
			{
				return profile;
			}
		}
	...
	private static string GetImportedTimelineProfileLabel(ImportedTimelineProfile profile)
	{
		var label = profile.ProfileName;
		if (!string.IsNullOrWhiteSpace(profile.Source.SourceJob))
		{
			label += $" [{profile.Source.SourceJob}]";
		}

		return label;
	}

	private static string GetImportedTimelineProfileTooltip(ImportedTimelineProfile profile)
	{
		...
		{
			builder.AppendLine(profile.Source.FightName);
		}

		if (!string.IsNullOrWhiteSpace(profile.Source.SourceJob))
		{
			builder.AppendLine($"Source job: {profile.Source.SourceJob}");
		}

		builder.Append($"Actions: {profile.Actions.Count}");
		return builder.ToString();
	}

	private static uint GetPreDutyConfigTerritory(Job job)
	{
		if (DataCenter.IsInDuty && Svc.ClientState.TerritoryType != 0)
		{
			_preDutyConfigTerritory = Svc.ClientState.TerritoryType;
		...
		if (_preDutyConfigTerritory != 0 && Array.Exists(territories, territory => territory.TerritoryType == _preDutyConfigTerritory))
		{
			return _preDutyConfigTerritory;
		}

		if (Service.Config.DutyCustomRotationTestTerritory != 0
		...
		var configuredTerritories = RotationUpdater.GetDutyCustomRotationTerritories(job, CombatType.PvE);
		foreach (var territoryType in configuredTerritories)
		{
			if (Array.Exists(territories, territory => territory.TerritoryType == territoryType))
			{
	...
	private static (uint TerritoryType, string DisplayName)[] GetPreDutyConfigTerritoryOptions()
	{
		...
		{
			_preDutyConfigTerritoryOptions = [];
			return _preDutyConfigTerritoryOptions;
		}
		...
		{
			var compare = string.Compare(left.DisplayName, right.DisplayName, StringComparison.CurrentCultureIgnoreCase);
			return compare != 0 ? compare : left.TerritoryType.CompareTo(right.TerritoryType);
		});
	...
	private static string GetTerritoryDisplayName(uint territoryType)
	{
		if (territoryType == Svc.ClientState.TerritoryType)
		{
			var currentTerritory = DataCenter.Territory;
			if (!string.IsNullOrWhiteSpace(currentTerritory?.ContentFinderName))
			{
				return currentTerritory.ContentFinderName;
			}

			if (!string.IsNullOrWhiteSpace(currentTerritory?.Name))
			{
