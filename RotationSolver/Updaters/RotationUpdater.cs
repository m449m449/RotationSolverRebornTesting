...
internal static class RotationUpdater
{
	...
	internal static void SetDutyCustomRotationChoice(uint territoryType, Job job, CombatType combatType, string? choice)
	{
		if (territoryType == 0)
		{
			return;
		...
		{
			Service.Config.DutyCustomRotationChoice[key] = choice;
		}
	}
	...
	internal static uint[] GetDutyCustomRotationTerritories(Job job, CombatType combatType)
	{
		...
		foreach (var pair in Service.Config.DutyCustomRotationChoice)
		{
			if (!string.IsNullOrWhiteSpace(pair.Value)
				&& TryParseDutyCustomRotationChoiceKey(pair.Key, out var territoryType, out var keyJob, out var keyCombatType)
				&& keyJob == job
	...
	private static string GetDesiredRotationName(Job job, CombatType combatType)
	{
		var globalChoice = combatType == CombatType.PvP ? Service.Config.PvPRotationChoice : Service.Config.RotationChoice;

		if (TryGetDutyCustomRotationChoice(Svc.ClientState.TerritoryType, job, combatType, out var dutyChoice))
		{
			return dutyChoice ?? globalChoice ?? string.Empty;
		}

		if (!DataCenter.IsInDuty
			&& combatType == CombatType.PvE
			&& TryGetDutyCustomRotationChoice(Service.Config.DutyCustomRotationTestTerritory, job, combatType, out var testChoice))
		{
			return testChoice ?? globalChoice ?? string.Empty;
		}

		return globalChoice ?? string.Empty;
	}

	private static string GetDutyCustomRotationChoiceKey(uint territoryType, Job job, CombatType combatType)
		=> $"{territoryType}:{(int)job}:{(int)combatType}";

	private static bool TryParseDutyCustomRotationChoiceKey(string key, out uint territoryType, out Job job, out CombatType combatType)
	{
		territoryType = 0;
		job = Job.ADV;
		...
		if (parts.Length != 3
			...
			|| !int.TryParse(parts[1], out var jobId)
			|| !Enum.IsDefined(typeof(Job), jobId)
			|| !int.TryParse(parts[2], out var combatTypeId)
			|| !Enum.IsDefined(typeof(CombatType), combatTypeId))
		{
			return false;
		}

		job = (Job)jobId;
		combatType = (CombatType)combatTypeId;
		return true;
	}
	...
	private static void UpdateCustomRotation()
	{
		...
		var desiredRotationName = GetDesiredRotationName(nowJob, curCombatType);
		...
		if (CustomRotationsLookup.TryGetValue(nowJob, out var validCustomRotations)) // Because default rotations exist, this *should* always have something; if not, no rotations
		{
			...
			if (validCustomRotations.TryGetValue(curCombatType, out var validCustomRotationsList))
			{
				...
				if (rotation == DataCenter.CurrentRotation
					&& DataCenter.CurrentRotation?.Job == nowJob
					&& DataCenter.CurrentRotation?.GetAttributes()?.Type == curCombatType)
				{
					return;
				}
